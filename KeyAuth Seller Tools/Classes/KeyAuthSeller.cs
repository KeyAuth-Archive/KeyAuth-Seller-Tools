...
namespace KeyAuthSeller
{
    public class SellerApi
    {
        ...
        public void SetSellerKey(string sellerKey)
        {
            ...
            ResponseStructure responseStructure = new();
            if(response == null)
            {
            ...
            LoadResponseStruct(responseStructure);
            _sellerKey=sellerKey;
        }
        #endregion
        ...
        public void LicenseGen(int expiry,string mask, int level, int amount)
        {
            ...
            string respon = Request(url);
            ResponseStructure? json = JsonConvert.DeserializeObject<ResponseStructure>(respon);
            if (json.Success)
            {
                int number = 1;
                string list = string.Empty;
                var jsonKeys = JsonConvert.DeserializeObject<KeysStructure>(respon);
                if (amount > 1)
                {
        ...
        /// <summary>
        ///  Creates a user from a license key
        /// </summary>
        /// <param name="userName">The user name you would like to give the new user</param
        /// <param name="key">The key you would like to use for the user</param>
        /// <param name="pass">The password you would like to set for the user</param>
        public void LicenseCreateUser(string userName,string license, string passWord)
        {
            string url = $"?sellerkey={_sellerKey}&type=activate&user={userName}&key={license}&pass={passWord}";
        ...
        public void LicenseViewAll()
        {
            ...
            if(json.Success)
            {
                ...
                jsonLicense.Total = $"{jsonLicense.Keys.Count} Licenses found";
                LoadLicenseStruct(jsonLicense);
            }
            LoadResponseStruct(json);
        }
        ...
        public void UserViewAll()
        {
            ...
            if(json.Success)
            {
                ...
                int number = 1;
                foreach (var user in jsonUsers.Users)
                {
                    if (user.Lastlogin == null)
                        user.Lastlogin = user.Createdate;
                    final += $"User: {number++}" +
                        ...
                        $"\nApp: {user.App}" +
                        $"\nOwner: {user.Owner}" +
                        $"\nCreation Date: {UnixTimeToDateTime(long.Parse(user.Createdate))}" +
                        $"\nLast Login: {UnixTimeToDateTime(long.Parse(user.Lastlogin))}" +
                        $"\nBan Reason: {user.Banned}" +
                        $"\nIP: {user.Ip}" +
                        $"\nCooldown: {user.Cooldown}" +
                        $"\n\n";
                }
                jsonUsers.List = final;
                jsonUsers.Total = $"{jsonUsers.Users.Count} Users Found";
        ...
        public void UserViewVars()
        {
            ...
            if (json.Success)
            {
                ...
                jsonUserVar.Total = $"{jsonUserVar.Vars.Count} User Vars Found";
        ...
        public void UserAllUserNames()
        {
            ...
            if (json.Success)
            {
                ...
                jsonUsers.List = list;
                jsonUsers.Total = $"{jsonUsers.Usernames.Count} Usernames Found";
                LoadUsersStruct(jsonUsers);
            }
            LoadResponseStruct(json);
        }
        ...
        public void SubViewAll()
        {
            ...
            if(json.Success)
            {
                ...
                jsonSubs.List = list;
                jsonSubs.Total = $"{jsonSubs.Subs.Count} Subscriptions found";
        ...
        {
            string seconds = timeOut.ToString();
            string url = $"?sellerkey={_sellerKey}&type=muteuser&user={userName}&timer={seconds}";
            var response = Request(url);
            var json = JsonConvert.DeserializeObject<ResponseStructure>(response);
            LoadResponseStruct(json);
        }
        ...
        {
            string url = $"?sellerkey={_sellerKey}&type=unmuteuser&name={userName}";
            var response = Request(url);
            var json = JsonConvert.DeserializeObject<ResponseStructure>(response);
            LoadResponseStruct(json);
        }
        ...
        public void CreateWebHook(string link, bool authed)
        {
            ...
            if (authed)
                ...
                needAuthed = 0;
            string url = $"?sellerkey={_sellerKey}&type=addwebhook&baseurl={link}&authed={needAuthed}";
            var response = Request(url);
            var json = JsonConvert.DeserializeObject<ResponseStructure>(response);
            LoadResponseStruct(json);
        }
        ...
        public void FileViewAll()
        {
            ...
            if (json.Success)
            {
                ...
                jsonFiles.List = list;
                jsonFiles.Total = $"{jsonFiles.Files.Count} Files Found";
        ...
        public void VariableViewAll()
        {
            ...
            if (json.Success)
            {
                ...
                jsonVars.List = list;
                jsonVars.Total = $"{jsonVars.Vars.Count} Variables Found";
        ...
        public void BlackListsDelete(string data, int type)
        {
            ...
            if (type == 0)
                ...
                ipHwid = "hwid";
            string url = $"?sellerkey={_sellerKey}&type=delblack&data={data}&blacktype={ipHwid}";
            var response = Request(url);
            var json = JsonConvert.DeserializeObject<ResponseStructure>(response);
            LoadResponseStruct(json);
        }
        ...
        public void BlackViewAll()
        {
            ...
            if (json.Success)
            {
                ...
                jsonBlacks.Total = $"{jsonBlacks.Subs.Count} Blacks Found";
        ...
        public void SettingsUpdate(bool appEnabled, bool hwidLockEnabled, string version, string webHook, string resellerStore, string disabledMsg, string usernameTakenMsg, string licenseInvalidMsg, string keyTakenMsg, string noSubMsg, string userInvalidMsg, string passInvalidMsg, string hwidMismatchMsg, string noActiveSubMsg, string blackedMsg, string expiredMsg, string sellixSecret)
        {
            ...
            if (hwidLockEnabled)
                ...
                hwidLock = "false";
            string url = $"?sellerkey={_sellerKey}&type=updatesettings&enabled={enabled}&hwidcheck={hwidLock}&ver={version}&webhook={webHook}&resellerstore={resellerStore}&appdisabled={disabledMsg}&usernametaken={usernameTakenMsg}&keynotfound={licenseInvalidMsg}&keyused={keyTakenMsg}&nosublevel={noSubMsg}&usernamenotfound={userInvalidMsg}&passmismatch={passInvalidMsg}&hwidmismatch={hwidMismatchMsg}&noactivesubs={noActiveSubMsg}&hwidblacked={blackedMsg}&keyexpired={expiredMsg}&sellixsecret={sellixSecret}";
            var response = Request(url);
            var json = JsonConvert.DeserializeObject<ResponseStructure>(response);
            if (json.Success)
                SettingsView();
        ...
        private static string? Request(string url)
        {
            ...
            else if (rawResponse.StatusCode == (HttpStatusCode)404)
            {
                Console.WriteLine(rawResponse.StatusCode);
                Error(error["message"].ToString());
        ...
        public LicenseClass license = new();
        ...
        public LicenseClass keys = new();
        public class LicenseKeyClass
        {
            public string Key { get; set; }
            public string List { get; set; }
        }
        ...
        public VariablesClass variables = new();
        ...
        public ResponseClass response = new();
